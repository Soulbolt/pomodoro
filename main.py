...
# ---------------------------- COUNTDOWN MECHANISM ------------------------------- #
def count_down(count):  #2 function to produce a count down utilizing window.after
    count_min = math.floor(count / 60)  # using math.floor rounds up to biggest number down?
    count_sec = count % 60
...
timer_text = canvas.create_text(100, 130, text="00:00", fill="white", font=(FONT_NAME, 25, "bold"))  #1 We assign this line of code a variable to call it
canvas.grid(column=1, row=1)

time_label = Label(text="Timer", bg=YELLOW, fg=GREEN, font=(FONT_NAME, 35, "bold"))
time_label.grid(column=1, row=0)
start_button = Button(text="Start", command=start_timer)  # Use command kw to call function
